# MEMORANDUM

**TO:** Coalition Partners & Philadelphia City Hall Officials
**FROM:** TOC Policy Analysis Team
**DATE:** January 15, 2026
**RE:** Transit-Oriented Communities Ordinance - Top 20 Stations & District Recommendations

---

## EXECUTIVE SUMMARY

This memo identifies **20 priority transit stations** for Transit-Oriented Communities (TOC) zoning reform in Philadelphia, based on recent multifamily development activity (2020-2025) within 1/4 mile of rapid transit.

**Key Findings:**

- **14,600 multifamily housing units** permitted within 1/4 mile of high-frequency transit since 2020
- **Composite scoring methodology:** Rankings balance development activity (50%) + transit frequency (25%) + regional jobs access (15%) + future market potential (10%)
- **Top 20 stations split into two categories:**
  - **8 Expansion Stations:** Already have 500-foot TOD overlays → expand to 1/4 mile
  - **12 New Stations:** No existing TOD → add new 1/4-mile zones
- **21 stations (32%)** sit on council district boundaries, requiring joint decisions
- **46th Street now #1 citywide** combining strong development + existing TOD + excellent transit frequency + superior regional jobs connectivity
- **Political strategy:** Expansion stations provide easier "on-ramp" before adding new stations

---

## I. TOP 20 STATIONS CITYWIDE

**Updated with composite scoring: Development (50%) + Transit Frequency (25%) + Jobs Access (15%) + Future Potential (10%)**

### **PART A: TOP EXPANSION STATIONS (8 stations)**
**Expand existing 500-foot TOD overlays → 1/4 mile**

These stations **already have TOD zoning under current city law** but only at 500-foot radius. Expanding to 1/4 mile aligns with mayor's proposal and captures significantly more development.

| Rank | Station | Line | District | Units (2020-2025) | Score | Boundary? |
|------|---------|------|----------|-------------------|-------|-----------|
| 1 | **46th Street** | MFL | 3 | 510 | 98.5 | - |
| 5 | **Berks** | MFL | 7 | 779 | 95.0 | - |
| 8 | **Huntingdon** | MFL | 7 | 156 | 62.1 | **D1/D7** |
| 9 | **52nd Street** | MFL | 3 | 118 | 58.8 | - |
| 11 | **Somerset** | MFL | 7 | 50 | 50.0 | **D1/D7** |
| 12 | **63rd Street** | MFL | 3 | 40 | 50.0 | **D3/D4** |
| 14 | **60th Street** | MFL | 3 | 22 | 47.7 | **D3/D4** |
| 17 | **56th Street** | MFL | 4 | 9 | 46.9 | **D3/D4** |

**Subtotal: 1,684 units across 8 stations (12% of citywide total)**

**Political Advantage:**
- TOD already exists at these stations - simply expanding radius
- Precedent established under current law
- Framed as "enhancing proven policy" rather than new regulation
- Mayor supports 1/4-mile distance

**Note on Boundaries:**
- **Huntingdon + Somerset (D1/D7):** Both in top 10 citywide; require joint approval by Council Members Squilla and Lozada
- **56th/60th/63rd (D3/D4):** Three consecutive stations on West Philly MFL; coordinate with Gauthier and Jones

---

### **PART B: TOP NEW STATIONS (12 stations)**
**Add new 1/4-mile TOD zones (no existing overlay)**

These stations currently have **no TOD zoning** but show strong development activity or excellent transit access/jobs connectivity justifying new 1/4-mile zones.

| Rank | Station | Line | District | Units (2020-2025) | Score | Boundary? |
|------|---------|------|----------|-------------------|-------|-----------|
| 2 | **Girard** | BSL | 5 | 816 | 97.0 | - |
| 3 | **Girard** | MFL | 5 | 816 | 96.4 | **D1/D5** |
| 4 | **York-Dauphin** | MFL | 7 | 764 | 95.5 | - |
| 6 | **Ellsworth-Federal** | BSL | 1 | 452 | 92.7 | **D1/D2** |
| 7 | **Erie** | BSL | 5 | 255 | 72.0 | **D5/D8** |
| 10 | **Susquehanna-Dauphin** | BSL | 5 | 77 | 51.2 | - |
| 13 | **Church** | MFL | 7 | 42 | 48.1 | - |
| 15 | **Olney Transportation Center** | BSL | 9 | 36 | 47.1 | **D8/D9** |
| 16 | **Hunting Park** | BSL | 5 | 45 | 46.9 | **D5/D8** |
| 18 | **Snyder** | BSL | 1 | 3 | 46.8 | **D1/D2** |
| 19 | **Cecil B Moore** | BSL | 5 | 17 | 46.6 | - |
| 20 | **Arrott Transportation Center** | MFL | 7 | 30 | 46.4 | - |

**Subtotal: 3,356 units across 12 stations (23% of citywide total)**

**Critical Insights:**

- **Both Girard stations (#2 and #3)** account for **1,632 combined units (11% of citywide total)** - the highest-impact TOC area citywide
- **46th Street (#1)** is now top-ranked combining strong development (510 units), existing TOD overlay, excellent transit frequency, and superior jobs access
- **York-Dauphin (#4)** has highest unit count (764) but composite score balances development with transit quality and jobs connectivity
- **8 of 12 new stations** sit on district boundaries, strengthening case for citywide ordinance vs. district-by-district approach

**Note on Boundaries:**
- **Ellsworth-Federal (D1/D2):** #6 citywide; South Philly's top station
- **Girard MFL (D1/D5), Erie (D5/D8):** High-priority North Philly corridor
- **Multiple D5/D8 stations:** Coordinate Richardson + Bass for North Philly BSL

---

## II. IMPLEMENTATION STRATEGY

### **Phase 1: Expansion Stations (Easier Political Lift)**

**Argument:** "These 8 MFL stations already have TOD zoning at 500 feet. The mayor proposed 1/4 mile. Let's align city policy with mayoral vision by expanding proven overlays."

**Captures:** 1,684 units
**Political Risk:** Low (precedent exists)
**Districts Affected:** 1, 3, 4, 7

### **Phase 2: New Stations (Full TOC Implementation)**

**Argument:** "Market data shows 12 additional stations deserve same treatment. Development is already happening—TOC ensures it serves transit riders and community goals."

**Captures:** 3,339 units (nearly 2x expansion stations)
**Political Risk:** Moderate (new regulation)
**Districts Affected:** 1, 2, 5, 7, 8, 9

### **Combined Impact: 5,040 units across 20 stations (34% of citywide total)**

**Note:** New composite scoring prioritizes stations with combination of development activity + high-frequency transit + regional jobs access, resulting in 46th Street moving to #1 and several stations shifting positions.

---

## III. TOP 4-5 STATIONS PER COUNCIL DISTRICT

District-specific recommendations for council members. Includes **BSL/MFL priority stations** plus **Regional Rail where BSL/MFL coverage is weak**.

---

### **DISTRICT 1** (Council Member: Mark Squilla)

**Top 5 All Modes (BSL/MFL + Trolleys + RR):**

| Rank | Station/Stop | Mode | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|--------------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **Girard** | MFL | 816 | 96.4 | **CITYWIDE #3** | NO | **D1/D5** |
| 2 | **Ellsworth-Federal** | BSL | 452 | 92.7 | **CITYWIDE #6** | NO | **D1/D2** |
| 3 | **Huntingdon** | MFL | 156 | 62.1 | **CITYWIDE #8** | **YES (500ft)** | **D1/D7** |
| 4 | **Girard Av & 2nd St** | Trolley | 386 | 59.3 | **Major corridor** | NO | - |
| 5 | **Girard Av & Front St** | Trolley | 386 | 59.3 | **Major corridor** | NO | - |

**Additional Priority Stations:**

| Station | Mode | Units | Notes |
|---------|------|-------|-------|
| **Somerset** | MFL | 50 | Citywide #11; **YES (500ft)**; **D1/D7 boundary** |
| Snyder | BSL | 3 | Tier 4; **D1/D2 boundary** |
| Tasker-Morris | BSL | 6 | Tier 4; **D1/D2 boundary** |
| Market East | RR | 0 | Center City; minimal residential |

**District Summary:**
- **Three boundary stations appear in D1's top 5**: Girard MFL (D1/D5), Ellsworth-Federal (D1/D2), and Huntingdon (D1/D7)
- **Girard MFL (#3 citywide)** is shared priority with District 5
- **Ellsworth-Federal (#6 citywide)** is South Philly's strongest station with 452 units
- **Huntingdon (#8 citywide)** has existing 500ft TOD, shared with District 7
- **Girard Avenue trolley stops** (Route 15) at 2nd St and Front St show 386 units each, extending the Girard multi-modal corridor into District 1
- **Most boundary stations of any district** (4 of top 5)

**Trolley Insight:**
Girard Avenue trolley extends from District 5's Girard BSL/MFL stations eastward through District 1 to the waterfront, creating a continuous multi-modal TOC corridor. Stops at 2nd St and Front St capture significant development along this eastern segment.

**Recommendation:** Include Girard MFL, Ellsworth-Federal, and Huntingdon (all high citywide priorities). **Add Girard Avenue trolley corridor** (Route 15) as multi-modal extension. Include Somerset (expand existing TOD). **Coordinate all boundary decisions** with CMs Richardson (D5), Johnson (D2), and Lozada (D7).

---

### **DISTRICT 2** (Council Member: Kenyatta Johnson)

**Top 5 All Modes (BSL/MFL + Trolleys + RR):**

| Rank | Station/Stop | Mode | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|--------------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **Ellsworth-Federal** | BSL | 452 | 92.7 | **CITYWIDE #6** | NO | **D1/D2** |
| 2 | **Oregon** | BSL | 0 | 43.9 | Tier 4 | NO | **D1/D2** |
| 3 | **AT&T Station** | BSL | 0 | 39.1 | Tier 4 | NO | - |
| 4 | **Island Av & Tanager St - F** | Trolley | 65 | 31.2 | Minor activity | NO | - |
| 5 | **Island Av & Buist Av** | Trolley | 65 | 31.2 | Minor activity | NO | - |

**Additional Priority Stations:**

| Station | Mode | Units | Notes |
|---------|------|-------|-------|
| Eastwick | RR | 0 | Far southwest; zero activity |
| Airport Terminals (4) | RR | 0 | Airport; non-residential area |

**District Summary:**
- **Ellsworth-Federal (#6 citywide)** is shared boundary station with District 1 - **South Philly's strongest TOC location**
- Oregon and AT&T (BSL) have **zero development activity** currently
- **Island Avenue trolley stops** show modest activity (65 units each) in far Southwest
- Airport and Eastwick RR stations are not viable residential TOC locations

**Trolley Insight:**
Island Avenue trolley stops (Route 36) serve Southwest Philadelphia with modest development activity, but significantly lower than corridors in other districts.

**Recommendation:** Include Ellsworth-Federal (shared D1/D2 priority). Include Oregon and AT&T for geographic coverage but recognize these are **not high-priority stations**. **Add Island Avenue trolley stops** for comprehensive Southwest coverage. **Coordinate with CM Squilla** on Ellsworth-Federal and Oregon decisions.

---

### **DISTRICT 3** (Council Member: Jamie Gauthier)

**Top 5 All Modes (BSL/MFL + Trolleys + RR):**

| Rank | Station/Stop | Mode | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|--------------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **46th Street** | MFL | 510 | 98.5 | **CITYWIDE #1** | **YES (500ft)** | - |
| 2 | **Lancaster Av & 41st St** | Trolley | 660 | 83.2 | **Major corridor** | NO | - |
| 3 | **Lancaster Av & 41st St** | Trolley | 653 | 82.6 | **Major corridor** | NO | - |
| 4 | **Lancaster Av & Wallace St** | Trolley | 619 | 79.7 | **Major corridor** | NO | - |
| 5 | **Lancaster Av & Haverford Av** | Trolley | 588 | 76.9 | **Major corridor** | NO | - |

**Additional Priority Stations:**

| Station | Mode | Units | Notes |
|---------|------|-------|-------|
| **52nd Street** | MFL | 118 | Citywide #9; existing 500ft TOD |
| **30th Street Station** | RR | 356 | #1 RR citywide; major hub |
| 49th Street | RR | 167 | Strong West Philly activity |
| 63rd Street | MFL | 40 | Existing 500ft TOD; **D3/D4 boundary** |
| 60th Street | MFL | 22 | Existing 500ft TOD; **D3/D4 boundary** |

**District Summary:**
- **LANCASTER AVENUE TROLLEY CORRIDOR IS TRANSFORMATIVE**: Ranks #2-5 are all Lancaster Ave stops with 660, 653, 619, and 588 units respectively
- **46th Street (#1 CITYWIDE)** anchors the district with highest composite score citywide
- **All four MFL stations have existing 500ft TOD** - perfect candidates for expansion to 1/4 mile
- **30th Street Station** is #1 Regional Rail station citywide (356 units) - major multi-modal hub
- **Total development at top 5 stations: 3,030 units** (Lancaster trolleys + 46th Street)

**Trolley Insight:**
Lancaster Avenue trolley stops are tightly clustered, creating a **continuous high-density corridor** from 41st to mid-30s. These stops show 600+ units EACH because their 1/4-mile buffers overlap significantly. The corridor as a whole represents one of the strongest TOC opportunities citywide.

**Recommendation:** Include all four MFL stations (expand existing TOD). **Add Lancaster Avenue trolley corridor** (Routes 10/11/13/34/36) as priority multi-modal TOC zone. **Add 30th Street Station and 49th Street** (Regional Rail). District 3 is TOC **model district** with strongest multi-modal coverage citywide.

---

### **DISTRICT 4** (Council Member: Curtis Jones Jr.)

**Top 5 All Modes (BSL/MFL + Trolleys + RR):**

| Rank | Station/Stop | Mode | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|--------------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **56th Street** | MFL | 9 | 46.9 | Tier 4 | **YES (500ft)** | **D3/D4** |
| 2 | **Girard Av & 51st St** | Trolley | 113 | 35.4 | **Major corridor** | NO | - |
| 3 | **Girard Av & 52nd St** | Trolley | 113 | 35.4 | **Major corridor** | NO | - |
| 4 | **Girard Av & 52nd St** | Trolley | 113 | 35.4 | **Major corridor** | NO | - |
| 5 | **Girard Av & 39th St** | Trolley | 110 | 35.1 | **Major corridor** | NO | - |

**Additional Priority Stations:**

| Station | Mode | Units | Notes |
|---------|------|-------|-------|
| **Overbrook** | RR | 111 | Strong activity |
| East Falls | RR | 46 | Walkable neighborhood center |
| Manayunk | RR | 44 | Walkable commercial district |
| Wissahickon | RR | 10 | Manayunk Line |

**District Summary:**
- **GIRARD AVENUE TROLLEY CORRIDOR DOMINATES**: Ranks #2-5 are all Girard Ave trolley stops with 110-113 units each
- **56th Street** (MFL) anchors with existing 500ft TOD on **D3/D4 boundary**
- **Girard corridor creates continuous TOC zone** through District 4's southern edge
- **Overbrook RR** (111 units) shows strong standalone activity
- **Total development at top 5 stations: 458 units** (Girard trolleys + 56th Street)

**Trolley Insight:**
Girard Avenue trolley stops (Route 15) are tightly clustered along the district's boundary with District 5, creating a **continuous multi-modal corridor** that complements the Girard BSL/MFL stations in District 5. These stops show 110+ units EACH due to overlapping 1/4-mile buffers.

**Recommendation:** Include 56th Street (MFL, expand existing TOD). **Add Girard Avenue trolley corridor** (Route 15) as priority TOC zone. **Add Overbrook, East Falls, and Manayunk** (Regional Rail) to provide District 4 with comprehensive multi-modal coverage. **Note:** Girard trolley corridor on D4/D5 boundary - coordinate with CM Richardson.

---

### **DISTRICT 5** (Council Member: Councilmember Gilmore Richardson)

**Top 5 All Modes (BSL/MFL + Trolleys + RR):**

| Rank | Station/Stop | Mode | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|--------------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **Girard** | BSL | 816 | 97.0 | **CITYWIDE #2** | NO | - |
| 2 | **Girard** | MFL | 816 | 96.4 | **CITYWIDE #3** | NO | **D1/D5** |
| 3 | **Girard Av & 16th St** | Trolley | 622 | 79.9 | **Major corridor** | NO | - |
| 4 | **Girard Av & 16th St** | Trolley | 622 | 79.9 | **Major corridor** | NO | - |
| 5 | **Girard Av & 17th St** | Trolley | 560 | 74.5 | **Major corridor** | NO | - |

**Additional Priority Stations:**

| Station | Mode | Units | Notes |
|---------|------|-------|-------|
| **Erie** | BSL | 255 | Citywide #7; **D5/D8 boundary** |
| Susquehanna-Dauphin | BSL | 77 | Tier 3 |
| Hunting Park | BSL | 45 | Tier 4; **D5/D8 boundary** |
| **Temple University** | RR | 322 | **#2 RR citywide** |
| Suburban Station | RR | 293 | Center City |

**District Summary:**
- **HIGHEST DEVELOPMENT ACTIVITY CITYWIDE**
- **GIRARD MULTI-MODAL CORRIDOR IS UNMATCHED**: Girard BSL (#2 citywide), Girard MFL (#3 citywide), and three Girard Ave trolley stops dominate top 5
- **Combined Girard corridor = 3,436 units** across BSL + MFL + Trolley stops
- **Composite scoring elevates both Girard stations to #2 and #3** due to excellent transit frequency + jobs access + development
- **Girard trolley stops** (Route 15) add 1,804 additional units along the corridor
- Erie (#7) is Tier 2 with 255 units
- Temple University RR is **#2 Regional Rail station citywide** (322 units)

**Trolley Insight:**
Girard Avenue has **three transit modes** (BSL, MFL, and Trolley Route 15) creating the most comprehensive multi-modal corridor in Philadelphia. The trolley stops at 16th and 17th Streets show 560-622 units EACH, complementing the two Girard rapid transit stations.

**Political Context:**
- CM Richardson characterized as "anti-development"
- **Counter-argument:** District already absorbed **3,436 units at Girard corridor alone**—development is happening regardless of zoning
- TOC doesn't CREATE pressure; it MANAGES existing market demand near transit

**Recommendation:** All 5 BSL/MFL stations are essential. **Add Girard Avenue trolley corridor** (Route 15) as critical multi-modal TOC zone. **Add Temple University and Suburban Station** (RR) for comprehensive coverage. **Coordinate boundary decisions** with CMs Squilla (D1) and Bass (D8).

---

### **DISTRICT 6** (Council Member: Kendra Brooks)

**Top 5 All Modes (BSL/MFL + Trolleys + RR):**

| Rank | Station/Stop | Mode | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|--------------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **Tioga** | MFL | 0 | 41.9 | Tier 4 | **YES (500ft)** | **D6/D7** |
| 2 | **Erie-Torresdale** | MFL | 0 | 41.3 | Tier 4 | **YES (500ft)** | **D6/D7** |
| 3 | **Richmond St & Allegheny Av** | Trolley | 41 | 29.1 | Minor activity | NO | - |
| 4 | **Bridesburg** | RR | 0 | 18.0 | Zero activity | NO | **D6/D7** |

**District Summary:**
- **Minimal transit coverage** overall
- Both MFL stations have existing 500ft TOD but **zero current development activity**
- **Richmond St & Allegheny Av trolley** (Route 25) shows modest activity (41 units)
- Bridesburg RR shows no activity
- **Three stations sit on D6/D7 boundary** - coordinate with CM Lozada

**Trolley Insight:**
Richmond Street trolley (Route 25) provides limited service to Northeast Philadelphia with modest development activity near Allegheny Avenue intersection.

**Recommendation:** Include Tioga and Erie-Torresdale (expand existing TOD) for **geographic coverage and equity**. **Add Richmond St & Allegheny Av trolley** for comprehensive coverage. Even without current strong activity, these stations deserve same upzoning opportunity as higher-income areas. Skip Bridesburg RR (no viability).

---

### **DISTRICT 7** (Council Member: Councilmember Quetcy Lozada)

**BSL/MFL Stations:**

| Rank | Station | Line | Units | Composite Score | Priority | Existing TOD? | Boundary? |
|------|---------|------|-------|-----------------|----------|--------------|-----------|
| 1 | **York-Dauphin** | MFL | 764 | 95.5 | **Tier 1 - Citywide #4** | NO | - |
| 2 | **Berks** | MFL | 779 | 95.0 | **Tier 1 - Citywide #5** | **YES (500ft)** | - |
| 3 | **Huntingdon** | MFL | 156 | 62.1 | **Tier 3 - Citywide #8** | **YES (500ft)** | **D1/D7** |
| 4 | **Somerset** | MFL | 50 | 50.0 | **Tier 3 - Citywide #11** | **YES (500ft)** | **D1/D7** |
| 5 | Church | MFL | 42 | 48.1 | Tier 4 | NO | - |

**Regional Rail Stations:**

*(District 7 has strong MFL coverage; RR not needed for top 5)*

**District Summary:**
- **Two of top 5 HIGHEST-PERFORMING STATIONS CITYWIDE** (York-Dauphin #4 and Berks #5)
- **York-Dauphin has highest unit count citywide (764)** but composite scoring balances raw development with transit quality + jobs access
- **4 of top 11 citywide stations** are in District 7
- **1,543 units at top 4 stations = 11% of citywide total**
- Huntingdon + Somerset sit on **D1/D7 boundary** - coordinate with CM Squilla

**Political Context:**
- CM Lozada characterized as "anti-development"
- **Counter-argument:** District already absorbed massive development (779 units at Berks alone)—market validated these TOC locations
- Development happening regardless; TOC ensures benefits reach residents

**Recommendation:** All 5 MFL stations are high priorities. **York-Dauphin and Berks are non-negotiable citywide priorities.** Coordinate Huntingdon/Somerset as joint D1/D7 decisions.

---

### **DISTRICT 8** (Council Member: Councilmember Cindy Bass)

**BSL/MFL Stations:**

| Rank | Station | Line | Units | Priority | Existing TOD? | Boundary? |
|------|---------|------|-------|----------|--------------|-----------|
| 1 | Wyoming | BSL | 17 | Tier 4 | NO | - |

**Regional Rail Stations:**

| Rank | Station | Units | Notes |
|------|---------|-------|-------|
| 1 | **North Philadelphia (Amtrak)** | **93** | **Top D8 station** |
| 2 | **North Philadelphia (SEPTA)** | **93** | Same location as Amtrak |
| 3 | **Chelten Avenue** | **84** | Germantown corridor |
| 4 | Germantown | 40 | Germantown commercial |
| 5 | Wayne Junction | 30 | North Philly |

**District Summary:**
- **Weak BSL coverage** (only Wyoming with 17 units)
- **Strongest Regional Rail district** - North Philly stations show moderate activity
- North Philadelphia Amtrak + SEPTA (same location, dual platforms) = 93 units each
- Chelten Avenue (84 units) serves Germantown commercial corridor

**Political Context:**
- CM Bass characterized as "anti-development"
- However, RR stations show **moderate market activity** that could support neighborhood revitalization

**Recommendation:** Include Wyoming (BSL). **Add North Philadelphia (Amtrak + SEPTA) and Chelten Avenue** (RR) as top priorities. These provide District 8 with 4-5 viable stations mixing BSL and RR.

---

### **DISTRICT 9** (Council Member: Anthony Phillips)

**BSL/MFL Stations:**

| Rank | Station | Line | Units | Priority | Existing TOD? | Boundary? |
|------|---------|------|-------|----------|--------------|-----------|
| 1 | **Olney Transportation Center** | BSL | 36 | Tier 4 | NO | **D8/D9** |
| 2 | Logan | BSL | 3 | Tier 4 | NO | **D8/D9** |

**Regional Rail Stations:**

| Rank | Station | Units | Notes |
|------|---------|-------|-------|
| 1 | Olney | 0 | Zero activity |
| 2 | Fern Rock TC | 0 | BSL/RR transfer; zero activity |

**District Summary:**
- **Limited transit coverage** overall
- Olney TC (36 units) shows some activity
- Both BSL stations sit on **D8/D9 boundary** - coordinate with CM Bass
- Fern Rock TC is BSL/RR transfer but shows **zero development activity** currently

**Recommendation:** Include Olney TC and Logan (BSL) for **geographic coverage**. Add Fern Rock TC (RR) for **multi-modal connectivity** despite current zero activity—creates option value if service frequency improves.

---

## IV. POLICY FRAMEWORK & POLITICAL STRATEGY

### **Core Argument: Housing Production IS the Community Benefit**

The market has validated TOC-eligible areas with **14,600 units** built near transit without TOC policy. This ordinance channels existing demand toward transit, climate, and equity goals.

**Community Benefits Delivered:**

1. **Housing Production:** 14,600 households with transit access = reduced car dependency, lower transportation costs
2. **Transit Ridership:** More riders = better service, more frequent trains, improved mobility for existing residents
3. **Economic Development:** Vibrant commercial corridors around transit nodes = local business growth, job creation
4. **Climate & Equity:** Reduced sprawl, lower VMT, concentrated investment in historically underserved areas

### **Inclusionary Zoning (IZ) Position**

- Not opposed in principle, but **only if fully funded** (Baltimore model with development subsidies)
- **Caution:** Unfunded IZ mandates risk reducing production (see MIN zones analysis)
- **Preference:** Allow market-rate TOC to proceed; use property tax increases to fund affordable housing trust

### **Community Benefits Agreements (CBAs): Not Recommended**

- CBAs create project-by-project barriers that slow housing production
- TOC should be **rule-based and predictable**, not discretionary
- Community benefits are embedded in policy itself

---

## V. IMPLEMENTATION PATHWAYS

### **Why Split Custody Analysis Strengthens the Citywide Case**

Recent analysis of boundary station development patterns reveals that **selective "top 15" approaches are almost as complex as citywide ordinances**, making comprehensive policy the more efficient path.

**Key Finding: Boundary Stations Concentrate at the Top**

Among the top 15 BSL/MFL stations by composite score, **8 stations (53%) sit on district boundaries**—significantly higher than the overall boundary rate (32% citywide). This means:

- Even a selective approach requires **8 districts + 8 boundary negotiations**
- Not meaningfully simpler than citywide policy
- Same political stakeholders involved either way

**The Split Percentage Problem:**

Split custody analysis reveals awkward development splits that invite political conflict:

| Station | Split | Political Question |
|---------|-------|-------------------|
| Girard MFL (D1/D5) | 63% D5, 37% D1 | Who controls decisions on a 60/40 split? |
| Huntingdon (D1/D7) | 62% D1, 38% D7 | Does minority district get veto power? |
| Erie (D5/D8) | 76% D5, 24% D8 | Should 24% district have equal say? |
| Ellsworth-Federal (D1/D2) | 90% D2, 10% D1 | Lopsided but still requires coordination |

Station-by-station negotiations over these percentages create friction. Citywide rules bypass this entirely.

**District 1 Becomes a Chokepoint:**

CM Squilla (D1) appears in **4 of 8 top-15 boundary stations**:
- Girard MFL (D1/D5)
- Ellsworth-Federal (D1/D2)
- Huntingdon (D1/D7)
- Somerset (D1/D7)

This concentrates power in one council member and creates bottlenecks for multiple high-priority stations.

**Trolley Corridors Don't Fit Station-by-Station Logic:**

The trolley analysis revealed continuous corridors, not isolated stations:
- **Lancaster Ave (D3)**: 4 of top 5 district stations are clustered trolley stops
- **Girard Ave (D4/D5)**: Multi-modal corridor spanning boundaries with BSL + MFL + Trolley
- **Corridor-based policy** (citywide TOC) fits this spatial reality better than station-by-station

**Bottom Line:**

Even a selective "top 15" approach requires coordinating **8 districts, 8 boundary splits, and buy-in from "anti-development" CMs** (Richardson D5, Lozada D7). That's not simpler than citywide—it's just messier.

**Strategic recommendation: Use boundary complexity as your argument FOR citywide ordinance:**

*"We could negotiate 21 separate boundary station agreements with complex development splits, or we could pass one simple citywide rule that treats everyone equally. Which makes more sense?"*

---

### **PLAN A: Citywide Ordinance (STRONGLY RECOMMENDED)**

**Scope:** All 20 priority stations (8 expansion + 12 new)
**Radius:** 1/4 mile per mayor's proposal

**Advantages:**
- **Simpler:** Single ordinance vote vs. 21 inter-district boundary negotiations
- **Equitable:** Same rules citywide; no neighborhoods left behind
- **Faster:** One decision vs. multi-year district-by-district process
- **Stronger:** 21 boundary stations (32% of total) require joint approvals anyway

**Path:** Coordinate with Mayor's Office; build coalition with pro-housing council members (Gauthier D3, Squilla D1, etc.)

---

### **PLAN B: Two-Phase District Approach (FALLBACK)**

If citywide ordinance fails, pursue two-phase strategy:

**Phase 1: Expansion Stations**
- 8 MFL stations with existing 500ft TOD → expand to 1/4 mile
- Affects Districts 1, 3, 4, 7 only
- Lower political lift (precedent exists)

**Phase 2: New Stations + Full Coverage**
- Add 12 new stations (Districts 1, 2, 5, 7, 8, 9)
- Include Regional Rail for Districts 4, 6, 8, 9
- Build on Phase 1 success

**Risk:** Prolonged timeline, uneven coverage, complex boundary negotiations

---

### **PLAN C: State-Level Reform (LONG-TERM)**

If local reform fails entirely:
- Pending state legislation mandates **1/2-mile TOC zones** around all SEPTA stations
- **Impact:** 17,000 units vs. 14,600 at 1/4 mile (+16%)
- **Advantage:** Bypasses hostile local councils, uniform statewide standards
- **Timeline:** Multi-year advocacy campaign

---

## VI. ADDRESSING OPPOSITION

### **Districts 5 & 7: High Activity, "Anti-Development" Members**

**Challenge:** Council Members Richardson (D5) and Lozada (D7) characterized as "anti-development," yet their districts contain **9 of top 10 citywide stations** and **4,821 units (33% of total)**.

**Response:**
- Emphasize development is **already happening** in their districts—TOC ensures it serves transit and community goals
- **Frame as protection:** "Better to have density on transit corridors than scattered through residential side streets"
- **Data point:** Both Girard stations (D5) = 1,632 units already built—TOC formalizes what market validated

### **Districts 2, 6, 9: Low Activity Areas**

**Challenge:** Minimal development near transit raises questions about TOC impact.

**Response:**
- TOC creates **option value** for future market shifts
- **Equity argument:** Low-income neighborhoods deserve same upzoning opportunities as high-income areas
- Regional Rail stations could become future nodes if service frequency improves

### **Boundary Stations: Use as Citywide Ordinance Argument**

**Message:** "21 boundary stations requiring inter-district coordination = strong case for simple citywide rules. Why negotiate 21 separate agreements when one ordinance treats everyone equally?"

---

## VII. NEXT STEPS

1. **Coalition Building:**
   - Share memo with pro-housing advocates, affordable developers, transit/climate groups
   - Coordinate messaging: "housing production as community benefit"

2. **City Council Outreach:**
   - **Priority meetings:** Gauthier (D3), Squilla (D1) - likely allies with strong station activity
   - **Boundary coordination:** Schedule joint meetings for D1/D2, D1/D7, D5/D8, D3/D4
   - **Data-driven engagement:** District-specific fact sheets with rankings above

3. **Mayor's Office Coordination:**
   - Align on 1/4-mile radius and priority stations
   - Discuss IZ funding options if mayor wants affordability component
   - Confirm support for citywide vs. district-by-district

4. **Public Messaging:**
   - Op-eds: "Market demands transit housing—let's legalize it"
   - Social media: Highlight top 10 stations as "no-brainer" TOC locations
   - Counter IZ/CBA critiques with MIN zones data

---

## APPENDICES

**A. Methodology:**
- Permit data: OpenDataPhilly, 2020-2025, R-2 occupancy (multifamily >2 dwellings)
- Clustering: 1/4 mile (402m) radius from station geocodes
- Scoring: Recent development (50%), market momentum (25%), DVRPC existing (15%), DVRPC future (10%)
- Existing TOD overlays: Philadelphia Zoning Code (13 MFL stations at 500 feet)

**B. Data Sources:**
- L&I building permits (1,898 R-2 permits citywide)
- DVRPC TOD Station Database (2018)
- SEPTA station locations and service frequencies
- Philadelphia Zoning Code (existing TOD overlays)
- City Council district boundaries (OpenDataPhilly)

**C. Key Files:**
- `tod_station_rankings_citywide.csv` - Complete rankings (30 BSL/MFL)
- `tod_station_rankings_by_district.csv` - District-specific rankings
- `regional_rail_analysis.json` - RR station clustering results
- `boundary_stations.json` - 21 boundary stations requiring joint decisions
- `all_modes_comparison.csv` - BSL/MFL/Trolleys/RR mode analysis

---

**For questions or additional analysis, contact the TOC Policy Analysis Team.**
